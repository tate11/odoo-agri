...
class Company(models.Model):
    ...
    def _create_activity_sequence(self):
        activity_vals = []
        for company in self:
            activity_vals.append({
                'name': '%s Activity Seq' % company.name,
                'code': 'agri.activity',
        ...
        if activity_vals:
            self.env['ir.sequence'].create(activity_vals)
    ...
    def _create_missing_activity_sequences(self):
        company_ids = self.env['res.company'].search([])
        company_has_activity_seq = self.env['ir.sequence'].search([
            ('code', '=', 'agri.activity')
        ]).mapped('company_id')
        company_todo_sequence = company_ids - company_has_activity_seq
        company_todo_sequence._create_activity_sequence()
    ...
    def create_missing_sequences(self):
        self._create_missing_activity_sequences()
        self._create_missing_production_record_sequences()

    def _create_per_company_sequences(self):
        self.ensure_one()
        self._create_activity_sequence()
